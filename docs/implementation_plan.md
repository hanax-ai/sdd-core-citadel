...
- [x] Verify against **real** provider APIs (not mocks) — completed 2026-07-30 across multiple live runs for Anthropic (`claude-opus-5`) and OpenAI (`gpt-5.3-codex`); both confirmed working end-to-end. **Gemini status is mixed, not fully live-verified:** `gemini-flash-latest` (the pre-upgrade default, unstructured text output) was live-confirmed working. The current default, `gemini-3.6-flash` with `thinking_level="HIGH"` and structured JSON (`response_schema`) output, is implemented and covered by mocked unit tests only — a real live call against this exact config has not succeeded, blocked by the free-tier daily quota exhausted during tonight's testing (see item 5 below). Treat live verification of the current Gemini default as **pending**, not complete. Five real bugs found and fixed along the way that the mocked test suite couldn't catch:
