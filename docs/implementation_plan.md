...
- [x] Verify against **real** provider APIs (not mocks) — completed 2026-07-30 across multiple live runs. All three providers confirmed working end-to-end; five real bugs found and fixed along the way that the mocked test suite couldn't catch:
  ...
  - **Known limitation, not a defect:** on this account's free-tier `gemini-flash-latest`, Gatekeeper sometimes flags a pre-existing condition as if the current patch introduced it, despite two rounds of prompt refinement (explicit scope-distinction instruction, then an explicit diff-format `+`/context-line explanation) — both real, generically-useful improvements to `agents/gatekeeper.py`'s `SYSTEM_PROMPT`/`format_review_prompt`, neither fully closed the gap on this test task. Root cause is believed to be a capability ceiling of the free-tier flash model, not the prompting — this account has zero quota on every pro-tier Gemini model tested. A stronger Gemini model would likely resolve it. Verdict on the demo task (documenting a hardcoded path) stayed `UNRESOLVED` for this reason even after Builder started producing fully correct patches; this is Gatekeeper doing its job conservatively, not a broken loop — do not chase this further without a Gemini billing upgrade.
